...
export function PainPointsSection({ onClick }: PainPointProps) {
    ...
    return <div className={`container mx-auto mt-32 lg:mt-56 max-w-7xl`}>
        <div className={`bg-blue-500 flex flex-col items-center py-20 rounded-3xl md:rounded-[80px]`}>
            <h2 className={`text-2xl lg:text-5xl font-bold w-4/5 leading-snug tracking-wide text-white text-center`}>
                Overcome Twitter Challenges with TweetCat
            </h2>
            <h3 className={`text-lg lg:text-2xl text-white font-medium text-center w-4/5 mt-6 leading-relaxed`}>
            ...
            <div
                className={`text-white flex flex-wrap gap-x-8 gap-y-8 justify-center mt-10 px-4`}>
                {painPoints.map((point) => (
                    <div key={point.text}
                         className="space-y-3 lg:w-3/12 border border-blue-400 rounded-2xl px-8 py-4">
                        <div>{point.icon}</div>
                        <div>{point.text}</div>
                    </div>
                ))}
            </div>
            <TwitterLoginButton onClick={onClick} name={`SAVE TIME`} wide className={`mt-12`}/>
        </div>
    </div>
        ;
}
